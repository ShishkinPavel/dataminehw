...
Usage: python final-project/run_pipeline.py --tag RPG   (из корня dataminehw/)

Агенты импортируются из hw1-hw4, все артефакты сохраняются в final-project/.
"""
...
def _import_agent(hw_dir: str, module_name: str, class_name: str):
    """Import agent class from its original hw directory."""
    module_path = os.path.join(ROOT, hw_dir, 'agents', f'{module_name}.py')
    spec = importlib.util.spec_from_file_location(f'{hw_dir}.{module_name}', module_path)
    mod = importlib.util.module_from_spec(spec)
    hw_full = os.path.join(ROOT, hw_dir)
    inserted = hw_full not in sys.path
    if inserted:
        sys.path.insert(0, hw_full)
    spec.loader.exec_module(mod)
    if inserted:
        sys.path.remove(hw_full)
...
импортирует классы из `hw1-hw4/agents/` напрямую (без копирования кода)
и передаёт данные через CSV-файлы.
...
- Агенты импортируются из hw1-hw4 через `importlib` — единый источник правды
...
def main(tag: str = 'Horror', top_n: int = 5, reviews_per_game: int = 100, hf_sample: int = 500):
    ...
    DataCollectionAgent = _import_agent('hw1-data-collection', 'data_collection_agent', 'DataCollectionAgent')
    DataQualityAgent = _import_agent('hw2-data-quality', 'data_quality_agent', 'DataQualityAgent')
    AnnotationAgent = _import_agent('hw3-annotation', 'annotation_agent', 'AnnotationAgent')
    ActiveLearningAgent = _import_agent('hw4-active-learning', 'al_agent', 'ActiveLearningAgent')
    ...
    ann_config = os.path.join(ROOT, 'hw3-annotation', 'config.yaml')
    ...
    al_config_path = os.path.join(ROOT, 'hw4-active-learning', 'config.yaml')
    ...
    al_config = os.path.join(ROOT, 'hw4-active-learning', 'config.yaml')
